...
interface UpdateNewsModalProps {
    ...
    onClose: () => void;
    onUpdate: (articleId: string, data: NewsFormData) => void;
}

const cantons = ["Basel", "Zurich", "Geneva", "Bern", "Lausanne", "Lugano"];

const UpdateNewsModal: React.FC<UpdateNewsModalProps> = ({ isOpen, article, onClose, onUpdate }) => {
    ...
    useEffect(() => {
        if (article && isOpen) {
            reset({
                title: article.title,
                clubScope: article.clubScope,
                status: article.status,
                geoScope: article.geoScope,
                canton: article.canton || "Zurich",
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-xs overflow-y-auto select-none">
            <div className="bg-card border border-card-border rounded-2xl max-w-xl w-full p-6 shadow-2xl space-y-5 my-8 animate-in fade-in zoom-in-95 duration-200">
                ...
                <div className="flex items-center justify-between border-b border-card-border/60 pb-3">
                    <h2 className="text-base font-bold text-title">Edit Press Article</h2>
                    <button
                        type="button"
                        onClick={onClose}
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                    ...
                    {/* Row 2: Club Scope & Editorial Status */}
